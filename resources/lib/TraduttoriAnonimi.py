...
class TraduttoriAnonimi:
    ...
    def __init__(self, baseURL="http://traduttorianonimi.it", showsListPath="elenco-serie/?S={0}",
                        ...
                        if subs is not None:
                            for tmp in subs:
                                if tmp is not None and "title" in tmp.attrs and "href" in tmp.attrs:
                                    ...
                                    if self._magicInt(x.group("season")) == season and self._magicInt(
                                            x.group("episode")) == episode:
