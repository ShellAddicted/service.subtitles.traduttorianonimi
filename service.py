...
class KodiLogHandler(logging.StreamHandler):
    ...
    def __init__(self):
        ...
        formatter = logging.Formatter(
            b'[{}]'.format(addon_id) + b'{%(levelname)s} %(name)s.%(funcName)s() -->> %(message)s')
            ...
            if results is not None:
                for result in results:
                    ...
                    for sub in subs:
                        ...
                        listitem.setProperty('sync', 'true' if checkSync(item["file_original_path"],
                                                                         os.path.basename(sub)) else 'false')
